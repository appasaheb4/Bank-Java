...
public class IncomeCommisstion extends JFrame {

	private JPanel contentPane;
	private JTable tableIncomeCommisstion;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					IncomeCommisstion frame = new IncomeCommisstion();
	...
	public IncomeCommisstion() {
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowOpened(WindowEvent e) {
				...
				int rowsCount = tableIncomeCommisstion.getRowCount();
				double sum = 0;
				double amttotal = 0;
				for (int i = 0; i < rowsCount; i++) {

					amttotal = amttotal + Double.valueOf(tableIncomeCommisstion.getValueAt(i, 5).toString());
				}
				textFieldTotalAmount.setText(String.valueOf(amttotal));
		...
		JLabel lblIncomeLoan = new JLabel("Income Commisstion");
		...
		label.setBounds(763, 47, 81, 17);
		panel.add(label);

		Date date = new Date();
		JDateChooser dateChooser = new JDateChooser();
		dateChooser.setDateFormatString("dd-MM-yyyy");
		dateChooser.setBounds(854, 47, 196, 20);
		...
		button.setIcon(new ImageIcon(IncomeCommisstion.class.getResource("/ImageButtonIcon/clac.png")));
		...
		tableIncomeCommisstion = new JTable();
		tableIncomeCommisstion.addMouseListener(new MouseAdapter() {
			@Override
			...
			private void che() {
				...
				String inc = null;
				int row = 0;
				int rows = tableIncomeCommisstion.getRowCount() - 1;

				for (int i = 0; i < tableIncomeCommisstion.getRowCount(); i++) {
					Boolean isChecked = Boolean.valueOf(tableIncomeCommisstion.getValueAt(i, 0).toString());
					if (isChecked) {
						inc = "select" + i;
						row = tableIncomeCommisstion.getSelectedRow();
						int col = tableIncomeCommisstion.getSelectedColumn();
						tableIncomeCommisstion.setValueAt((Object) s, i, 0);
						if (inc.equals("select0")) {
							tableIncomeCommisstion.setValueAt((Object) s1, 0, 0);
						} else if (i == rows) {
							tableIncomeCommisstion.setValueAt((Object) s1, rows, 0);
						} else if (i != rows) {
							tableIncomeCommisstion.setValueAt((Object) s, rows, 0);

						}

					} else if (i >= 0) {
						if (row != 0) {
							tableIncomeCommisstion.setValueAt((Object) s, 0, 0);
							tableIncomeCommisstion.setValueAt((Object) s1, row, 0);
						}
					}
		...
		tableIncomeCommisstion.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
		tableIncomeCommisstion.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		tableIncomeCommisstion.setModel(new DefaultTableModel(new Object[][] {},
				new String[] { "Select", "Sr No", "Date", "Name", "Account No", "Amount" }) {
		...
		tableIncomeCommisstion.getColumnModel().getColumn(0).setPreferredWidth(50);
		tableIncomeCommisstion.getColumnModel().getColumn(0).setMinWidth(50);
		tableIncomeCommisstion.getColumnModel().getColumn(1).setResizable(false);
		tableIncomeCommisstion.getColumnModel().getColumn(1).setPreferredWidth(50);
		tableIncomeCommisstion.getColumnModel().getColumn(1).setMinWidth(50);
		tableIncomeCommisstion.getColumnModel().getColumn(2).setPreferredWidth(80);
		tableIncomeCommisstion.getColumnModel().getColumn(2).setMinWidth(80);
		tableIncomeCommisstion.getColumnModel().getColumn(3).setPreferredWidth(150);
		tableIncomeCommisstion.getColumnModel().getColumn(3).setMinWidth(150);
		tableIncomeCommisstion.getColumnModel().getColumn(5).setPreferredWidth(80);
		tableIncomeCommisstion.getColumnModel().getColumn(5).setMinWidth(80);
		showdata();
		scrollPane.setViewportView(tableIncomeCommisstion);
		...
		lblTotalAmount.setBounds(864, 569, 117, 17);
		contentPane.add(lblTotalAmount);

		textFieldTotalIntsert = new JTextField();
		textFieldTotalIntsert.setVisible(false);
		...
		textFieldTotalAmount.setBounds(991, 569, 153, 23);
		...
		button_1.setIcon(new ImageIcon(IncomeCommisstion.class.getResource("/ImageButtonIcon/Exit.png")));
		...
		button_1.setBounds(10, 558, 115, 38);
		contentPane.add(button_1);

		JButton btnSearch = new JButton("Search");
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				model1 = (DefaultTableModel) tableIncomeCommisstion.getModel();
				...
				try {
					...
					String loqu = "SELECT * FROM banksystem.commissiona_c where CommisstionPaidDate between '" + datemin
							+ "' AND '" + datemax + "';";
					...
					while (rs.next()) {
						...
						String data4 = rs.getString("CommisstionAmt");
	...
	public void filter(String quey) {
		...
		tableIncomeCommisstion.setRowSorter(tr);
