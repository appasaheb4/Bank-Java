...
public class ReportBank extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ReportBank frame = new ReportBank();
	...
	public ReportBank() {
		...
		JLabel lblNewLabel = new JLabel("Bank Report");
		...
		panel_1.setBounds(10, 176, 1134, 415);
		...
		tablebankowner
				.setModel(
						new DefaultTableModel(new Object[][] {},
								new String[] { "Select", "SrNoMaster", "SrNo", "Tran Date", "A/C No", "Bank Name",
										"Amount", "Tran Type", "Mode of Payment", "Cheque No", "Cheque Name",
										"Trans Details" }) {
		...
		tablebankowner.getColumnModel().getColumn(5).setPreferredWidth(150);
		tablebankowner.getColumnModel().getColumn(5).setMinWidth(150);
		...
		tablebankowner.getColumnModel().getColumn(7).setMinWidth(80);
		tablebankowner.getColumnModel().getColumn(8).setPreferredWidth(80);
		tablebankowner.getColumnModel().getColumn(8).setMinWidth(8);
		...
		tablebankowner.getColumnModel().getColumn(11).setPreferredWidth(80);
		tablebankowner.getColumnModel().getColumn(11).setMinWidth(80);
		showdata();
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					for (int i = 0; i < tablebankowner.getRowCount(); i++) {
						...
						if (chckbxseall.isSelected()) {
							java.io.InputStream file = getClass().getResourceAsStream("/Reports/Bankall.jrxml");
							allinonereport r7 = new allinonereport("Bank Report", file);

							break;
						} else {
							if (che) {
								accno4 = tablebankowner.getValueAt(i, 4).toString();

								HashMap<String, String> para = new HashMap<String, String>();
								para.put("Srno", accno4);

								java.io.InputStream file = getClass().getResourceAsStream("/Reports/Bank.jrxml");
								allinonereport r7 = new allinonereport("Bank Report", file, para);
								break;
							}
						}
					}
		...
		button_1.setIcon(new ImageIcon(ReportBank.class.getResource("/ImageButtonIcon/Report4d.png")));
		...
		btnExit.setIcon(new ImageIcon(ReportBank.class.getResource("/ImageButtonIcon/Exit.png")));
		btnExit.setHorizontalAlignment(SwingConstants.LEADING);
		btnExit.setFont(new Font("Tahoma", Font.BOLD, 14));
		btnExit.setBounds(165, 655, 129, 38);
		contentPane.add(btnExit);

		chckbxseall = new JCheckBox("Select All");
		chckbxseall.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					for (int i = 0; i < tablebankowner.getRowCount(); i++) {
						Boolean isChecked = Boolean.valueOf(tablebankowner.getValueAt(i, 0).toString());
						if (chckbxseall.isSelected()) {
							tablebankowner.setValueAt((Object) s1, i, 0);
						} else {
							tablebankowner.setValueAt((Object) s, i, 0);
						}
					}
		...
		chckbxseall.setBounds(32, 610, 130, 23);
		contentPane.add(chckbxseall);
	}
	...
	public void showdata() {
		try {
			...
			while (rs.next()) {
				...
				Object[] row = { Boolean.FALSE, data0, data1, data2, accno, data3, data4, data5, data6, data7, data8,
						data9 };
				model1 = (DefaultTableModel) tablebankowner.getModel();
				model1.addRow(row);
