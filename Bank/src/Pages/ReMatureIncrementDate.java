...
public class ReMatureIncrementDate extends JDialog {
	...
	public static void main(String[] args) {
		try {
			ReMatureIncrementDate dialog = new ReMatureIncrementDate();
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowOpened(WindowEvent e) {
				...
				try {
					...
					String query = "Select sum(Amount),Interst from banksystem.dailyrecurring where SrNoMaster=" + Srno
							+ ";";
					ps = (PreparedStatement) conn.prepareStatement(query);
					rs = ps.executeQuery();
					while (rs.next()) {
						Amount = (rs.getDouble(1));
						interst = rs.getDouble("Interst");
					}
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						try {
							conn = (Connection) DBConnection.getConnection();
							String up = "UPDATE banksystem.recurring set Date=?,Updateamtdate=?, Amount=?, Intrest=?, IntrestAmt=?, MaturedAmount=?, Duration=?, Days=? where AccountNo='"
									+ Recurring.acno + "'";
							...
							ps.setDouble(3, Double.valueOf(textFieldAmount.getText()));
							ps.setDouble(4, Double.valueOf(textFieldInterst.getText()));
							ps.setDouble(5, Double.valueOf(textFieldInterstAmt.getText()));
							ps.setDouble(6, Double.valueOf(textFieldMaturityAmt.getText()));
							ps.setString(7, comboBoxDouration.getSelectedItem().toString());
							ps.setString(8, textField_Days.getText());
							int i = ps.executeUpdate();
							if (i > 0) {
								String del = "delete from banksystem.dailyrecurring where AccountNo='" + Recurring.acno
										+ "'";
								...
								if (ii > 0) {
									...
									ps = (PreparedStatement) conn.prepareStatement(
											"insert into banksystem.dailyrecurring(SrNOMaster, SrNO, Date, CustomerName, AccountNo, AgentName, Amount, Interst, InterestAmt, MaturedAmt, Duration, Days) values("
													+ mama + "," + 1 + ",'" + (java.sql.Date) d + "','"
													+ Recurring.cusname + "','" + Recurring.acno + "','"
													+ Recurring.agname + "','" + textFieldAmount.getText() + "','"
													+ textFieldInterst.getText() + "','" + textFieldInterstAmt.getText()
													+ "','" + textFieldMaturityAmt.getText() + "','"
													+ comboBoxDouration.getSelectedItem().toString() + "','"
													+ textField_Days.getText() + "')");
	...
	public void clac() {
		...
		textFieldInterstAmt.setText(String.valueOf(interstsum));
		double maturamt = amt + interstsum;
		textFieldMaturityAmt.setText(String.valueOf(maturamt));
	}
}
